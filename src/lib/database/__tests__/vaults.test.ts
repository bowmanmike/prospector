import { setupTestDatabase, mockVault } from "./test-utils";
...
describe("VaultQueries", () => {
  ...
  describe("getAll", () => {
    it("should return all vaults ordered by created_at DESC", async () => {
      const vault1 = await queries.vaults.create({ ...mockVault, path: "/path1" });
      const vault2 = await queries.vaults.create({ ...mockVault, path: "/path2" });
