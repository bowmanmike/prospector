...
describe('Home Component', () => {
  ...
  describe('Initial Load', () => {
    it('should render the main heading', () => {
      ...
      expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Prospector')
      expect(screen.getByText('AI-powered knowledge discovery for your Obsidian vault')).toBeInTheDocument()
    })

    it('should show loading state initially', async () => {
      mockVaultStorage.getVaultSettings.mockImplementation(() => new Promise(() => {})) // Never resolves
    ...
    it('should show vault selection UI when no vault exists', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Select your Obsidian vault directory to get started')).toBeInTheDocument()
      })
      
      expect(screen.getByRole('button', { name: 'Select Vault Directory' })).toBeInTheDocument()
      expect(screen.getByText('Your browser will request permission to access files. This is required to read your vault.')).toBeInTheDocument()
    })
    ...
    it('should load existing vault settings on mount', async () => {
      const mockSettings = {
        ...
        name: 'test-vault',
        files: [
          { path: 'test-vault/.obsidian/config.json', lastModified: 123, size: 50, type: 'application/json' }
        ],
      ...
      await waitFor(() => {
        expect(screen.getByText('Your vault is connected and ready to use')).toBeInTheDocument()
      })
      
      expect(screen.getByText('test-vault')).toBeInTheDocument()
      expect(screen.getByText('✓ Valid Obsidian vault detected')).toBeInTheDocument()
    })
    ...
    it('should show invalid vault warning for non-Obsidian directories', async () => {
      const mockSettings = {
        ...
        name: 'invalid-vault',
        files: [
          { path: 'invalid-vault/file.txt', lastModified: 123, size: 50, type: 'text/plain' }
        ],
        lastAccessed: 1234567890000
      }
      ...
      await waitFor(() => {
        expect(screen.getByText('⚠ No .obsidian folder found - this may not be a valid Obsidian vault')).toBeInTheDocument()
      })
    })
  })
  ...
  describe('Vault Selection', () => {
    it('should handle directory selection', async () => {
      ...
      // Mock file selection
      const fileInput = screen.getByRole('button', { name: 'Select Vault Directory' }).parentElement?.querySelector('input[type="file"]')
      ...
      await waitFor(() => {
        expect(mockVaultStorage.saveVaultFiles).toHaveBeenCalledWith(mockFiles, 'test-vault')
      })
  ...
  describe('Connected Vault Actions', () => {
    ...
    it('should handle change vault action', async () => {
      ...
      const fileInput = changeButton.parentElement?.querySelector('input[type="file"]')
      expect(fileInput).toBeInTheDocument()
    })
  })
  ...
  describe('Error Handling', () => {
    it('should handle storage errors gracefully', async () => {
      mockVaultStorage.getVaultSettings.mockRejectedValue(new Error('Storage error'))
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Error loading existing vault:', expect.any(Error))
    ...
    it('should handle clear vault errors', async () => {
      ...
      mockVaultStorage.getVaultSettings.mockResolvedValue(mockSettings)
      mockVaultStorage.isValidObsidianVault.mockResolvedValue(true)
      mockVaultStorage.clearVaultSettings.mockRejectedValue(new Error('Clear error'))
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Error clearing directory:', expect.any(Error))
    ...
    it('should handle vault validation errors', async () => {
      ...
      mockVaultStorage.getVaultSettings.mockResolvedValue(mockSettings)
      mockVaultStorage.isValidObsidianVault.mockRejectedValue(new Error('Validation error'))
      ...
      await waitFor(() => {
        expect(consoleSpy).toHaveBeenCalledWith('Error validating vault:', expect.any(Error))
      })
